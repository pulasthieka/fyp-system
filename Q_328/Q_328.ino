...
//  Serial.print('I'+String(IR)+'X'+'R'+String(RED)+'X');
  ...
  TakePPG();
  getimp(0x02, 0x71, 0x0B,'A'); //5k
  TakePPG();
  getimp(0x04, 0xE2, 0x17,'B'); //10k   
  TakePPG();
  getimp(0x18, 0x6A, 0x73,'C'); //50k
...
void FivekTenk()
{
#define gain5k 9638582
#define gain10k 9643946
  unsigned int imfive;
  unsigned int refive;
  unsigned int imten;
  unsigned int reten;
  ...
  refive = ADread(0x94) << 7;
  refive = refive << 1 | ADread(0x95);
  refive = negcor(refive);
  imfive = ADread(0x96) << 7;
  imfive = imfive << 1 | ADread(0x97);
  imfive = negcor(imfive);
  //values ready
  imp5k = gain5k / (sqrt(pow(double(refive), 2) + pow(double(imfive), 2)));
  // Serial.println(imfive);
  //Serial.println(refive);
  //  Serial.print("5k: ");
  Serial.println(imp5k);

}

void TewkFifk()
{
#define gain20k 2291021 //9627804
#define gain50k 9580963
  unsigned int imtew;
  unsigned int retew;
  unsigned int imfif;
  unsigned int refif;
  ...
  retew = ADread(0x94) << 7;
  retew = retew << 1 | ADread(0x95);
  retew = negcor(retew);
  imtew = imtew << 1 | ADread(0x97);
  imtew = negcor(imtew);
  //values ready
  imp20k = gain20k / (sqrt(pow(double(retew), 2) + pow(double(imtew), 2)));
  //
  //  #Serial.print("20k: ");
  Serial.print(imp20k);
  Serial.print("\t");
  //Serial.println(retew);
  //Serial.println(imtew);

  ADwrite(0x80, 0b00110001); //increment PGA=1
  while ((ADread(0x8F) & 0b00000010) != 2 ) {
    delayMicroseconds(1);
  }
  refif = ADread(0x94) << 7;
  refif = refif << 1 | ADread(0x95);
  refif = negcor(refif);
  imfif = ADread(0x96) << 7;
  imfif = imfif << 1 | ADread(0x97);
  imfif = negcor(imfif);
  //values ready
  imp50k = gain50k / (sqrt(pow(double(refif), 2) + pow(double(imfif), 2)));
  // Serial.print("50k: ");
  Serial.println(imp50k);
  //  Serial.println("----=----");
  //Serial.println(refif);
  //Serial.println(imfif);
}
